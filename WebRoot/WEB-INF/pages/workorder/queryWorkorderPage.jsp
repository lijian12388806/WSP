...
<form id="pagerForm" method="post"
	...
	<!-- 查询条件 -->
	<input type="hidden" value="${querycustomername}"
		id="querycustomername" name="querycustomername">
	<input type="hidden" value="${querycustlinkman}" id="querycustlinkman"
		name="querycustlinkman">
	<input type="hidden" value="${querygeneratemode}"
		id="querygeneratemode" name="querygeneratemode">
	...
	<input type="hidden" value="${querycallerno}" id="querycallerno"
		name="querycallerno">
	<input type="hidden" value="${queryordertype}" id="queryordertype"
		name="queryordertype">
	<input type="hidden" value="${orderCode}" id="orderCode"
		name="orderCode">
	...
	<input type="hidden" value="${querywostatus}" id="querywostatus"
		name="querywostatus">
	<input type="hidden" value="${querycompleteperson}"
		id="querycompleteperson" name="querycompleteperson">
			...
			<c:forEach var="item" items="${sessionScope.workorderList}">
				<tr target="slt_uid" rel="${item.id}">
					...
					<td>
						...
						<a class="btnEdit"
							href="${applicationScope.rootpath}editWorkOrder.action?workOrderKey=${item.id}&orderCode=${orderCode}&tabId=${tabId}"
							target="navTab" rel="workorderCardNav" reloadFlag=1 title="修改工单"><span>修改工单</span>
						</a>
						<c:if test="${not empty item.temp8 && not empty item.temp7}">
						<a class="btnMusic"
							href="javascript:Player('http://172.16.1.100:8080/Ipcc/Audio?AudioParam=ContactID=${item.temp8}:AgentID=${item.temp7}','play_${orderCode}');">播放</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
