...
/**
 * <p>
 * String Utility Class. This is used to encode passwords programmatically. It
 * also encodes string for inserting in database
 * </p>
 * <p>
 * <a href="StringUtil.java.html"><i>View Source</i></a>
 * </p>
 * 
 * @author Sergey Zubtsovskiy <a
 *         href="mailto:sergey.zubtsovskiy@blandware.com">&
 *         lt;sergey.zubtsovskiy@blandware.com&gt;</a>
 * @author Andrey Grebnev <a
 *         href="mailto:andrey.grebnev@blandware.com">&lt;andrey
 *         .grebnev@blandware.com&gt;</a>
 * @author Matt Raible <a
 *         href="mailto:matt@raibledesigns.com">&lt;matt@raibledesigns.
 *         com&gt;</a>
 * @version $Revision: 1.24 $ $Date: 2006/08/03 10:07:33 $
 */
public class StringUtil {
	 ...
	 * cookies.
	 * <p>
	 * * This is weak encoding in that anyone can use the decodeString routine
	...
	public static String escapeSpecial(String inputString) {
		if (inputString == null)
			return null;
	...
	public static String quoteEscape(String html) {
		if (html == null) {
			return null;
		}
		StringBuffer buf = new StringBuffer((int) (html.length() * 1.1));
	 ...
	 */
	public static String createTemplateName(String identifier, String field,
			String locale) {
	...
	public static String shortenString(String s, int requiredLength) {
		if (s != null && s.length() > requiredLength) {
			...
			if (space > 0 || lineFeed > 0 || tab > 0) {
				int cut = space > lineFeed ? (space > tab ? space : tab)
						: (lineFeed > tab ? lineFeed : tab);
				s = s.substring(0, cut);
			}
			s += "...";
		}
	 ...
	 *            Number to complete
	 * @param signs
	 *            Length of number to reach
	 * @return Adds zero-by-zero from to the left of number until its length
	 *         will be greater or equal than specified number of signs. For
	 *         example if number = 67 and signs = 3 are specified, result string
	 *         will be "067" and so on
	 */
	...
	public static String getRandomToken(int len) {
		...
		for (int i = 0; i < len; i++) {
			result.append(Character.forDigit(Integer.parseInt(RandomStringUtils
					.random(36)), 36));
		}
	...
	public static String convertArrayToString(String[] selectIDS) {
		StringBuffer b = new StringBuffer();
		int len = selectIDS.length;
		for (int i = 0; i < len; i++) {
			b.append(selectIDS[i]);
			if (i != len - 1) {
				b.append(",");
			}
		}
	...
	public static String convertListToString(List<String> listStr,String separator) {
		StringBuffer b = new StringBuffer();
	...
	public static void main(String[] args) {
		// System.out.println(new StringUtil().encodePassword("123456", "MD5"));
		// System.out.println(new StringUtil().encodePassword("123456", "MD5"));
		// System.out.println(new StringUtil().encodePassword("123456", "MD5"));
		// new StringUtil().escape();
