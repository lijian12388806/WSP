...
public class CnToSpell {
	 ...
	 * @param ascii
	 *            int 字符对应的ASCII
	 * @return String 拼音,首先判定ASCII是否>0&<160,假如是返回对应的字符,
	 * 
	...
	public static String getFullSpell(String cnStr) {
		...
		if("深圳".equals(cnStr)||"深圳市".equals(cnStr)){
			return "shenzhen";
		}
		if("重庆".equals(cnStr)||"重庆市".equals(cnStr)){
			return "chongqing";
		}
		if("什么".equals(cnStr)){
			return "shenme";
		}
		if(cnStr.endsWith("省")||cnStr.endsWith("市")){
			cnStr = cnStr.substring(0, cnStr.length()-1);
		}
		if(cnStr.endsWith("自治区") || cnStr.endsWith("自治州") || cnStr.endsWith("自治县")){
			cnStr = cnStr.substring(0, cnStr.length()-3);
		}
		if(cnStr.endsWith("地区")){
			cnStr = cnStr.substring(0, cnStr.length()-2);
		}
