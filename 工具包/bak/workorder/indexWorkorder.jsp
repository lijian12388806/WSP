...
function checkaction(v){ 
	if(v==0){ 
	document.getElementById("iactionForm").action="${applicationScope.rootpath}saveIndexWorkOrder.action?navTabId=woStatistics&callbackType=closeCurrent&orderCode=yjd";
	}else if(v == 1){ 
	document.getElementById("iactionForm").action="${applicationScope.rootpath}saveIndexWorkOrder.action?navTabId=woStatistics&callbackType=closeCurrent&orderCode=${workOrderForm.wostatus}"; 
	}else if(v == 2){
	document.getElementById("iactionForm").action="${applicationScope.rootpath}saveIndexWorkOrder.action?navTabId=woStatistics&callbackType=closeCurrent&orderCode=ywc";
	}
	$('#iactionForm').submit(); 
	//$("#woStatistics").loadUrl("ListIndexWorkorder.action",{},null); 
	} 
//-->
</script>
<div class="pageContent">
	<form method="post" action="" class="required-validate pageForm"
		...
		id="iactionForm" name="iactionForm">
		...
		<div class="formBar">
			<ul>
				...
				<c:if test="${workOrderForm.wostatus != 'yjd'}">
					<li>
						<div class="buttonActive">
				...
				<c:if test="${workOrderForm.wostatus == 'wcl'}">
					<li>
						<div class="buttonActive">
							<div class="buttonContent" onclick="checkaction(1);">
								<button type="button">
				...
				<c:if test="${workOrderForm.wostatus == 'dhxcl'}">
					<li>
						<div class="buttonActive">
...
<div class="pageContent"
	<c:if test="${empty workOrderKey || workOrderForm.wostatus == 'wcl'}">style="display:none"</c:if>>
	<div class="panelBar">
		<ul class="toolBar">
			<li>
				<a class="add"
					href="${applicationScope.rootpath}addIndexOrderdealinfo.action?workOrderKey=${workOrderKey}"
				...
				<tr target="slt_uid" rel="${item.id}">
					...
					<td>
						...
						<a class="btnEdit"
							...
							target="navTab" rel="editIndexOrderdealinfoCardNav" reloadFlag=1
