...
function checkaction(v){ 
	if(v==0){ 
	document.getElementById("eactionForm").action="${applicationScope.rootpath}saveWorkOrder.action?navTabId=workorderdhxclLiNav&callbackType=closeCurrent&orderCode=yjd"; 
	}else if(v == 1){ 
	document.getElementById("eactionForm").action="${applicationScope.rootpath}saveWorkOrder.action?navTabId=workorderdhxclLiNav&callbackType=closeCurrent&orderCode=dhxcl"; 
	}else if(v == 2){
	document.getElementById("eactionForm").action="${applicationScope.rootpath}saveWorkOrder.action?navTabId=workorderdhxclLiNav&callbackType=closeCurrent&orderCode=ywc";
		}
	$('#eactionForm').submit(); 
	} 
//-->
	...
	<form method="post" action="" class="required-validate pageForm"
			...
			<fieldset>
				<p>
					...
					<input name="workOrderForm.callerno" id="workOrderForm.callerno"
						type="hidden" value="${workOrderForm.callerno}" />
				...
				<p>
					...
					<input name="workOrderForm.workorderno"
						id="workOrderForm.workorderno" type="hidden"
			...
			<fieldset>
				...
				<p>
					...
					<input name="workOrderForm.custphoneno"
						...
						lookupGroup="workOrderForm" disabled="disabled"/>
				</p>
				<p>
					<label>
						QQ号码 ：
					...
					<input name="workOrderForm.temp1" id="workOrderForm.temp1"
						...
						value="${workOrderForm.temp1}" lookupGroup="workOrderForm" disabled="disabled"/>
				</p>
				<p>
					<label>
						电子邮箱地址 ：
					...
					<input name="workOrderForm.emailaddr" id="workOrderForm.emailaddr"
						...
						value="${workOrderForm.emailaddr}" lookupGroup="workOrderForm" disabled="disabled"/>
				</p>
				<p>
					<label>
						所属城市 ：
				...
				<dl class="nowrap">
					...
					<dd>
						<textarea name="workOrderForm.custaddr"
							...
							lookupGroup="workOrderForm" disabled="disabled">${workOrderForm.custaddr}</textarea>
					</dd>
				</dl>
			</fieldset>
		</div>
		<div class="formBar">
			<ul>
				...
				<li>
					<div class="buttonActive">
						<div class="buttonContent" onclick="checkaction(2);">
							<button type="button">
								已完成
							</button>
						</div>
...
<div class="pageContent"
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li>
				<a class="add"
					href="${applicationScope.rootpath}addDhxclOrderdealinfo.action?workOrderKey=${workOrderKey}"
			...
			<c:forEach var="item" items="${sessionScope.orderdealList}">
				<tr target="slt_uid" rel="${item.id}">
					...
					<td>
						...
						<a class="btnEdit"
							href="${applicationScope.rootpath}editDhxclOrderdealinfo.action?orderdealinfoKey=${item.id}&workOrderKey=${workOrderKey}"
							target="navTab" rel="editDhxclOrderdealinfoCardNav" reloadFlag=1
